  ...
  object AddressDto {
    ...
    def fromAddress(domainObj: Address): AddressDto =
      AddressDto(
        ...
        domainObj.addressLine2.map(_.value),
        domainObj.addressLine3.map(_.value),
        domainObj.addressLine4.map(_.value),
...
/// See "Serializing Records and Choice Types Using Maps" in chapter 11
  sealed trait PlaceOrderEventDto
  ...
  object PlaceOrderEventDto {
    ...
    def fromDomain(order: PlaceOrderEvent): PlaceOrderEventDto = order match {
      case op: OrderPlaced          => OrderPlacedDto.fromDomain(op)
      case bop: BillableOrderPlaced => BillableOrderPlacedDto.fromDomain(bop)
      case as: OrderAcknowledgmentSent =>
        OrderAcknowledgmentSentDto.fromDomain(as)
    }
  }
//===============================================
  ...
  object PlaceOrderErrorDto {
    ...
    def fromDomain(domainObj: PlaceOrderError): PlaceOrderErrorDto =
      domainObj match {
        ...
        case RemoteServiceError(info, ex) =>
          PlaceOrderErrorDto(
            "RemoteServiceError",
            s"${info.name}: ${ex.getMessage}"
          )
      }
  }
}
